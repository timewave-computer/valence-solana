...
use valence_kernel::{
    guards::Guard,
    state::{CreateSessionParams, Session, SessionSharedData},
    SessionScope,
    OperationBatch,
};
...
impl<'a> SessionHandle<'a> {
    ...
    pub fn execute_session_operations_instruction(
        &self,
        ...
        let mut instruction_data = Vec::new();
        instruction_data.extend_from_slice(&[1u8; 8]); // Placeholder discriminator for execute_session_operations
...
impl<'a> SessionHandle<'a> {
    ...
    /// This method is deprecated - use execute_session_operations_instruction instead
    #[deprecated(note = "Use execute_session_operations_instruction with OperationBatch")]
    pub fn execute_instruction(&self, _operation: Vec<u8>) -> Instruction {
        unimplemented!("Use execute_session_operations_instruction with OperationBatch instead")
    }
