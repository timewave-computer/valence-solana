...
use crate::{
    errors::KernelError, 
    operations::{self, KernelOperationBatch, KernelOperation},
    state::{Session, GuardData, CpiAllowlistAccount}
};
...
pub fn execute_session_operations(
    ...
    for operation in batch.operations.iter() {
        ...
        match operation {
            KernelOperation::BorrowAccount { account, mode } => {
            ...
            KernelOperation::ReleaseAccount { account } => {
                session.release_account(account, &ctx.accounts.clock)?;
            }
            
            KernelOperation::UpdateMetadata { metadata } => {
                session.set_metadata(*metadata, &ctx.accounts.clock);
            }
            
            KernelOperation::CallProgram { manifest_index, data, account_indices } => {
            ...
            KernelOperation::Custom { program_id, discriminator: _, data: _ } => {
                ...
                // Extension point for shard-specific operations
                // Shard implementations would handle the CPI to program_id
...
fn process_borrow_account<'info>(
    ...
    if mode & operations::ACCESS_MODE_WRITE != 0 {
        require!(account_info.is_writable, KernelError::AccountNotWritable);
    }
...
fn process_invoke_program<'info>(
    ...
    batch: &KernelOperationBatch,
    remaining_accounts: &[AccountInfo<'info>],
    cpi_allowlist: &CpiAllowlistAccount,
) -> Result<()> {
...
pub struct ExecuteOperations<'info> {
    ...
    pub cpi_allowlist: Box<Account<'info, CpiAllowlistAccount>>,
    ...
    // and to the operation processor for CallProgram.
}