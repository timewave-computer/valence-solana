// Comprehensive unit tests for valence-kernel
#[cfg(test)]
mod tests {
    use crate::{
        guards::*,
    ...
    #[test]
    fn test_apu_program_validation() {
        let mut compiled = SerializedGuard {
    ...
    fn test_apu_program_execution() {
        ...
        let ctx = GuardEvaluationContext {
            session: &session,
            caller: &caller,
            clock: &clock,
            operation,
        ...
        let guard = SerializedGuard {
            opcodes: vec![GuardOp::Terminate],
            cpi_manifest: vec![],
        };
        assert!(execute_serialized_guard(&guard, &ctx).unwrap());
        ...
        assert!(!execute_serialized_guard(&guard, &ctx).unwrap()); // Falls off end = false
        ...
        let ctx_wrong = GuardEvaluationContext {
            session: &session,
            caller: &wrong_caller,
            clock: &clock,
            operation,
        ...
        let guard_with_fail = SerializedGuard {
            opcodes: vec![
        ...
        let result = execute_serialized_guard(&guard_with_fail, &ctx_wrong);
    ...
    fn test_guard_expiry_checks() {
        ...
        let guard_expired = SerializedGuard {
            opcodes: vec![
        ...
        assert!(!execute_serialized_guard(&guard_expired, &ctx).unwrap());
    }
    ...
    fn test_guard_not_before() {
        ...
        let future_time = 2000000000i64;
        let guard_too_early = SerializedGuard {
            opcodes: vec![
        ...
        assert!(!execute_serialized_guard(&guard_too_early, &ctx).unwrap());
    }
    ...
    fn test_guard_usage_limit() {
        ...
        // Test at limit
        let guard_at_limit = SerializedGuard {
            opcodes: vec![
        ...
        assert!(!execute_serialized_guard(&guard_at_limit, &ctx).unwrap());
    }
    ...
    fn test_guard_complex_logic() {
        ...
        assert!(execute_serialized_guard(&guard_or, &ctx_high_usage).unwrap());
    }
    ...
    fn test_cpi_allowlist() {
        ...
        let mut allowlist = CpiAllowlistAccount {
    ...
    fn test_session_bitmap_edge_cases() {
        ...
        let account1 = Pubkey::new_unique();
        assert!(session.borrow_account(account1, ACCESS_MODE_READ, &clock).is_ok());
        assert_eq!(session.borrowed_accounts[0].mode, ACCESS_MODE_READ);
        assert!(session.borrowed_accounts[0].can_read());
        assert!(!session.borrowed_accounts[0].can_write());
        
        // Release and reborrow with write
        assert!(session.release_account(&account1, &clock).is_ok());
        assert!(session.borrow_account(account1, ACCESS_MODE_WRITE, &clock).is_ok());
        assert!(session.borrowed_accounts[0].can_write());
        assert!(!session.borrowed_accounts[0].can_read());
        
        // Test read-write mode
        let account2 = Pubkey::new_unique();
        assert!(session.borrow_account(account2, ACCESS_MODE_READ_WRITE, &clock).is_ok());
    ...
    fn test_session_has_borrowed_performance() {
        ...
        for i in (0..8).step_by(2) {
            assert!(session.borrow_account(accounts[i], ACCESS_MODE_READ, &clock).is_ok());
        }
    ...
    fn test_session_shared_data_methods() {
        ...
        assert!(!shared_data.has_flag(SessionSharedData::FLAG_CROSS_SHARD));
