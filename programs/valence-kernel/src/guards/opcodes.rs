...
// ================================
// Serialized Guard Opcode
// ================================

/// A serialized guard ready for execution
#[derive(Clone, Debug, AnchorSerialize, AnchorDeserialize)]
pub struct SerializedGuard {
    /// Flattened opcodes
    pub opcodes: Vec<GuardOp>,
    /// CPI instructions that can be invoked
    pub cpi_manifest: Vec<CpiCallEntry>,
}

impl SerializedGuard {
    /// Calculate space needed for this serialized guard
    pub fn calculate_space(&self) -> usize {
        8 + // discriminator
    ...
    /// Validate the serialized guard
    pub fn validate(&self) -> Result<()> {
        // Check opcode count
        require!(
            self.opcodes.len() <= 255,
            crate::errors::KernelError::GuardDataTooLarge
        );
    ...
    /// Create a new serialized guard with CPI manifest
    pub fn new_with_manifest(opcodes: Vec<GuardOp>, cpi_manifest: Vec<CpiCallEntry>) -> Result<Self> {
        let guard = Self {
            opcodes,
            cpi_manifest,
        };
