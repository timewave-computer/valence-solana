...
pub struct SessionSharedData {
    ...
    /// Shard-specific custom data storage
    /// Use this field for your shard's session state
    pub custom_data: [u8; 32],
    
    /// Reserved space for future functionality
    pub reserved_bytes: [u8; 220],
}
...
impl SessionSharedData {
    ...
    pub const FLAG_CROSS_SHARD: u8 = 1 << 3;
    ...
    pub fn enter_protected_section(&mut self) -> Result<()> {
        require!(!self.is_entered(), KernelError::InvalidStateTransition);
    ...
    pub fn check_and_increment_cpi_depth(&mut self) -> Result<()> {
        require!(self.cpi_depth < Self::MAX_CPI_DEPTH, KernelError::CrossProgramInvocationDepthExceeded);
        self.cpi_depth += 1;
        Ok(())
    }
