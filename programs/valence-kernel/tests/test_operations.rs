...
async fn test_account_validation() {
    ...
    let result2 = context.execute_session_operations(
        &session_keypair,
        &guard_data_keypair,
        &allowlist_keypair,
    ...
    let result3 = context.execute_session_operations(
        &session_keypair,
        &guard_data_keypair,
        &allowlist_keypair,
        batch_invalid_index,
...
async fn test_access_modes() {
    ...
    assert_eq!(session.borrowed_accounts[1].mode, ACCESS_MODE_WRITE);
    assert!(!session.borrowed_accounts[1].can_read());
    assert!(session.borrowed_accounts[1].can_write());
    
    assert_eq!(session.borrowed_accounts[2].mode, ACCESS_MODE_READ_WRITE);
