...
use valence_kernel::{
    PROGRAM_ID, CreateSession, Session, CreateSessionParams, SessionScope,
    SessionSharedData, BorrowedAccount, GuardData, SerializedGuard,
    KernelOperationBatch, KernelOperation, Initialize, GuardOp,
    SessionOperation, SessionContextScope
};

// Type alias for compatibility
type OperationBatch = KernelOperationBatch;
...
async fn test_complete_session_lifecycle() {
    ...
    let serialized_guard = SerializedGuard {
        opcodes: vec![
            GuardOp::CheckOwner,
    ...
    context.execute_session_operations(
        &session_keypair,
        &guard_data_keypair,
        &allowlist_keypair,
        batch,
