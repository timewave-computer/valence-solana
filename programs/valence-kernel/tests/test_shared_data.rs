...
async fn test_shared_data_feature_flags() {
    ...
    for (i, flags) in [
        ...
        (SessionSharedData::FLAG_CROSS_SHARD, "cross_shard"),
    ].iter().enumerate() {
        let guard_data_keypair = Keypair::new();
        let session_keypair = Keypair::new();
...
async fn test_shared_data_integration() {
    ...
    shared_data.set_flag(SessionSharedData::FLAG_DEBUG | SessionSharedData::FLAG_CROSS_SHARD);
    ...
    let session_params = CreateSessionParams {
        scope: SessionContextScope::Global,
        guard_data: guard_data_keypair.pubkey(),
        bound_to: None,
        shared_data,
        metadata: [99u8; 64],
    ...
    assert!(session.shared_data.has_flag(SessionSharedData::FLAG_CROSS_SHARD));
