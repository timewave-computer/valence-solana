...
mod fuzz_tests {
    ...
    fn fuzz_guard_validation() {
        ...
        for guard in test_guards {
            ...
            // Try to compile it
            if let Ok(compiled) = crate::guards::compile_guard(&guard) {
    ...
    fn fuzz_compiled_guard_opcodes() {
        ...
        for opcodes in test_opcode_sets {
            if let Ok(guard) = CompiledGuard::new(opcodes) {
                let _ = guard.validate();
            }
        }
    }
    ...
    fn test_guard_validation_fuzz() {
        fuzz_guard_validation();
    }
    
    #[test]
    fn test_compiled_guard_opcodes_fuzz() {
        fuzz_compiled_guard_opcodes();
    }
    ...
    #[test]
    fn test_compiled_guard_validation_repeated() {
