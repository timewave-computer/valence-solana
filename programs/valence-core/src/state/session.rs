...
pub enum SessionScope {
    ...
    User,
    /// Protocol-wide operations with global authority
    Global,
    ...
    Dao(Pubkey),
    /// Custom scope for protocol-specific contexts
    Custom {
        /// Your protocol's program ID
        protocol: Pubkey,
...
pub struct Session {
    ...
    pub owner: Pubkey,
    /// Protocol that created this session
    pub protocol: Pubkey,
    ...
    pub borrowed_accounts: [BorrowedAccount; 8],
...
impl Session {
    ...
    pub fn new(
        ...
        Self {
            ...
            borrowed_accounts: [BorrowedAccount::EMPTY; 8],
            borrowed_bitmap: 0,
        }
    }
    ...
    pub fn is_global(&self) -> bool {
        matches!(self.scope, SessionScope::Global)
    }
    ...
    pub fn increment_usage(&mut self, clock: &Clock) -> Result<()> {
        self.usage_count = self
            .usage_count
            .checked_add(1)
            .ok_or(crate::errors::ValenceError::UsageLimitExceeded)?;
    ...
    pub fn borrow_account(&mut self, account: Pubkey, mode: u8, clock: &Clock) -> Result<()> {
        ...
        while bitmap != 0 {
            if bitmap & 1 != 0 && self.borrowed_accounts[i].address == account {
                return Err(crate::errors::ValenceError::AccountAlreadyBorrowed.into());
            }
            bitmap >>= 1;
            i += 1;
        }
        ...
        if free_slots == 0 {
            return Err(crate::errors::ValenceError::BorrowCapacityExceeded.into());
        }
        ...
        self.borrowed_accounts[slot] = BorrowedAccount {
            address: account,
    ...
    pub fn has_borrowed(&self, account: &Pubkey) -> Option<&BorrowedAccount> {
    ...
    pub fn release_account(&mut self, account: &Pubkey, clock: &Clock) -> Result<()> {
        // Fast path: no accounts borrowed
        if self.borrowed_bitmap == 0 {
            return Err(crate::errors::ValenceError::AccountNotBorrowed.into());
        }
        ...
        let i = slot.ok_or(crate::errors::ValenceError::AccountNotBorrowed)?;
        
        // Clear the slot - O(1) operation
        self.borrowed_accounts[i] = BorrowedAccount::EMPTY;
        // Clear bit in bitmap
    ...
    pub const LEN: usize = 8 + // discriminator
        ...
        8 + // created_at
        8 + // updated_at
        8 * (32 + 8 + 1) + // borrowed_accounts array (8 * BorrowedAccount size)
        1; // borrowed_bitmap
}
