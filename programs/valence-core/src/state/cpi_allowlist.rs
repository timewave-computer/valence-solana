...
impl CPIAllowlist {
    ...
    pub fn is_allowed(&self, program_id: &Pubkey) -> bool {
        ...
        if is_system_program(program_id) || is_spl_program(program_id) {
            return true;
        }
    ...
    pub fn add_program(&mut self, program_id: Pubkey) -> Result<()> {
        require!(
            ...
            ValenceError::AllowlistFull
        );
        
        require!(
            !self.allowed_programs.contains(&program_id),
            ValenceError::ProgramAlreadyAllowed
        );
    ...
    pub fn remove_program(&mut self, program_id: &Pubkey) -> Result<()> {
        let index = self.allowed_programs
            ...
            .ok_or(ValenceError::ProgramNotAllowed)?;
...
fn is_spl_program(program_id: &Pubkey) -> bool {
    // Common SPL programs
