    ...
    impl Tracker {
        ...
        pub fn track(&mut self, category: &str, units: u64) -> Result<()> {
            ...
            if self.used > self.budget {
                return Err(crate::errors::ValenceError::ComputeBudgetExceeded.into());
            }

            // Update category breakdown using fixed array
            let category_id = Self::category_to_id(category);
        ...
        fn category_to_id(category: &str) -> u8 {
            // Simple hash: sum of bytes modulo 255, ensuring non-zero
            let sum: u32 = category.bytes().map(|b| b as u32).sum();
            ((sum % 255) + 1) as u8
        }
...
impl OperationCost {
    ...
    pub fn estimate(&self, account_count: usize) -> u64 {
...
pub use compute_budget::{costs, Tracker};
