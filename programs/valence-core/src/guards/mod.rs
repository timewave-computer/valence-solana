// Guard system for authorization and access control in valence-core
...
pub const MAX_GUARD_DEPTH: u8 = 3;

/// Maximum size for external guard data to prevent DoS
pub const MAX_EXTERNAL_GUARD_DATA_SIZE: usize = 256;

/// Maximum number of accounts an external guard can require
pub const MAX_EXTERNAL_GUARD_ACCOUNTS: usize = 8;
    ...
    pub fn validate(&self) -> Result<()> {
        match self {
            Guard::External { data, required_accounts, .. } => {
                if data.len() > MAX_EXTERNAL_GUARD_DATA_SIZE {
                    return Err(crate::errors::ValenceError::GuardDataTooLarge.into());
                }
                if required_accounts.len() > MAX_EXTERNAL_GUARD_ACCOUNTS {
                    return Err(crate::errors::ValenceError::GuardDataTooLarge.into());
                }
            }
