...
use super::opcodes::{GuardOp, CompiledGuard};
...
pub fn evaluate_compiled_guard(
    guard: &CompiledGuard,
        ...
        match op {
            ...
            GuardOp::Invoke { manifest_index } => {
                ...
                let (returned_program, return_data) = solana_program::program::get_return_data()
                    .ok_or(ValenceError::ExternalGuardNoReturnData)?;
                
                require!(
                    returned_program == entry.program_id,
                    ValenceError::ExternalGuardInvalidReturn
                );
