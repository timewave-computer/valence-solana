...
pub struct UpdateTemplateParams {
    ...
    pub required_destination: Option<Option<Pubkey>>,
    pub expiration_seconds: Option<Option<u64>>,
}
...
pub fn handler(ctx: Context<UpdateTemplate>, params: UpdateTemplateParams) -> Result<()> {
    ...
    // Update required destination (can be set to None)
    if let Some(required_destination) = params.required_destination {
        template.required_destination = required_destination;
    }
    
    // Update expiration seconds (can be set to None)
    if let Some(expiration_seconds) = params.expiration_seconds {
        template.expiration_seconds = expiration_seconds;
    }
