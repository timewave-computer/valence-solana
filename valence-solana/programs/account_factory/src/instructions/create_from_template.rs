...
pub fn handler(ctx: Context<CreateFromTemplate>, params: CreateFromTemplateParams) -> Result<()> {
    ...
    match template.account_type {
        ...
        2 => {
            // Create Single-Use Account
            msg!("Creating Single-Use Account from template: {}", template.template_id);
            // Logic for creating the Single-Use Account via CPI would go here
            
            // Apply template overrides if provided
            let required_destination = params.override_required_destination.or(template.required_destination);
            let expiration_seconds = params.override_expiration_seconds.or(template.expiration_seconds);
            
            if let Some(expiration) = expiration_seconds {
                msg!("Account will expire after {} seconds", expiration);
            }
            
            if let Some(destination) = required_destination {
                msg!("Required destination set to: {}", destination);
            }
        },
        _ => {
