...
pub fn handler(ctx: Context<BatchUpdate>, params: BatchUpdateParams) -> Result<()> {
    ...
    // Validate batch size
    if params.updates.len() + params.deletes.len() > 20 {
    ...
    msg!("Batch update processed with {} updates and {} deletes", params.updates.len(), params.deletes.len());
